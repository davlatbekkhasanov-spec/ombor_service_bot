...
def resolve_db_path(*, env_key: str = "DB_PATH", default_filename: str = "orders.db") -> str:
...
def migrate_legacy_db(target_path: str, *extra_legacy: str) -> str | None:
    ...
    candidates: list[str] = []
    for p in (*extra_legacy, base, "orders.db", os.path.join("/app", base), os.path.join("/app", "orders.db")):
...
def bootstrap_persistence(
    db_path: str,
    *,
    legacy_names: tuple[str, ...] = ("orders.db",),
) -> dict:
